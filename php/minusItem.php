<?php 
	...
	if ($quantity <= 1) {
		$stmt = "DELETE from cart_list where buyer_id = 1 and product_id = $1";
		pg_prepare($link,"deleteFromCart", $stmt);
		pg_execute($link,"deleteFromCart", array($product_id));
		...
		pg_close($link);
		echo json_encode(array('result' => 'deleted', 'sum' => $sum));
	} else {
		...
		pg_close($link);
		echo json_encode(array('result' => $quantity - 1, 'sum' => $sum));
	}
